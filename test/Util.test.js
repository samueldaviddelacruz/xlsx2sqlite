...
const {mapRowsToSqliteColumns,getSheetsFromExcelWorkBook} = require('../lib/Util/Utils')
    ...
    describe('mapRowsToSqliteColumns',()=>{
        ...
        it('should return an Array of sql types columns',async () =>{
            ...
            const sheetJsons = getSheetsFromExcelWorkBook('test.xlsx').map( (data) => {
                return {sheetName:data.name, jsonRows:xlsx.utils.sheet_to_json(data.sheet,{defval:""})}
            }  );
            console.log(sheetJsons)
            /*
            const columns = mapRowsToSqliteColumns();
